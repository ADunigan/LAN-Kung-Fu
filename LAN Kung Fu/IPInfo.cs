...
namespace LAN_Kung_Fu
{
    ...
    public class IPInfo
    {
        ...
        public static List<IPInfo> GetInterfaceIPInfo(IPAddress ip)
        {            
            try
            {
                var ipinfo = new List<IPInfo>();

                foreach (var arp in GetARPResult(ip).Split(new char[] { '\n', '\r' }))
                {
                    ...
                    if (!string.IsNullOrEmpty(arp))
                    {
                        ...
                        if (pieces.Length == 3)
                        {
                            ipinfo.Add(new IPInfo(pieces[1], pieces[0]));
                        }
                    }
                }
        ...
        public static List<IPInfo> GetIPInfo()
        {
            try
            {
                ...
                foreach (var arp in GetARPResult(null).Split(new char[] { '\n', '\r' }))
                {
                    // Parse out all the MAC / IP Address combinations
                    if (!string.IsNullOrEmpty(arp))
                    {
        ...
        /// </summary>
        /// <returns></returns>
        private static string GetARPResult(IPAddress ip)
        {
            Process p = null;
            ...
            try
            {
                if(ip != null)
                {
                    p = Process.Start(new ProcessStartInfo("arp", "-a -N " + ip.ToString())
                    {
                        CreateNoWindow = true,
